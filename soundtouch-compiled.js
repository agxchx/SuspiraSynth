// SoundTouchJS Compiled - All modules in one file for browser use.
(function(global) {
    'use strict';
  
    // ===== Contenido de noop.js =====
    const noop = function () {
      return;
    };
  
    // ===== Contenido de FifoSampleBuffer.js =====
    ...
    // ===== Contenido de testFloatEqual.js =====
    const testFloatEqual = function (a, b) {
      return (a > b ? a - b : b - a) > 1e-10;
    };
  
    // ===== Contenido de WebAudioBufferSource.js =====
    class WebAudioBufferSource {
      ...
      extract(target, numFrames = 0, position = 0) {
        ...
        let i = 0;
        for (; i < numFrames; i++) {
          if (i + position >= left.length) break;
    ...
    // ===== Contenido de getWebAudioNode.js =====
    const getWebAudioNode = function (
      ...
      node.onaudioprocess = (event) => {
        ...
        if (framesExtracted === 0 && typeof filter.onEnd === 'function') {
          filter.onEnd();
        }
        let i = 0;
    ...
    // ===== Contenido de AbstractFifoSamplePipe.js =====
    ...
    // ===== Contenido de RateTransposer.js =====
    ...
    class Stretch extends AbstractFifoSamplePipe {
      ...
      seekBestOverlapPositionStereo() {
        ...
        for (let i = 0; i < this.seekLength; i++) {
          let correlation = this.calculateCrossCorrelationStereo(2 * i, this.refMidBuffer);
      ...
      seekBestOverlapPositionStereoQuick() {
        ...
        for (let scanCount = 0; scanCount < 4; scanCount++) {
          ...
          while (_SCAN_OFFSETS[scanCount][j]) {
            let tempOffset = correlationOffset + _SCAN_OFFSETS[scanCount][j];
            if (tempOffset >= this.seekLength) break;
            let correlation = this.calculateCrossCorrelationStereo(2 * tempOffset, this.refMidBuffer);
            if (correlation > bestCorrelation) {
              bestCorrelation = correlation;
    ...
    // ===== EXPORTAR CLASES PRINCIPALES AL OBJETO GLOBAL (window) =====
    global.AbstractFifoSamplePipe = AbstractFifoSamplePipe;
    global.RateTransposer = RateTransposer;
    global.SimpleFilter = SimpleFilter;
    global.Stretch = Stretch;
    global.SoundTouch = SoundTouch;
    global.WebAudioBufferSource = WebAudioBufferSource;
    global.PitchShifter = PitchShifter;
    global.getWebAudioNode = getWebAudioNode;
